...
- [ ] 관리자 전용 로그인 시스템 구현 (이메일+비밀번호+Google Authenticator TOTP)
- [ ] Navbar에 관리자 로그인 버튼 추가
- [ ] /login 페이지 구현
- [ ] /admin 백오피스 로그인 후 접근 가능하도록 연결
...
## 백오피스 비밀번호 변경 UI
- [ ] 백오피스에 비밀번호 변경 페이지/모달 추가 (현재 비밀번호 + 새 비밀번호)
...
## 섹션 순서 변경
- [ ] 직추천 수익 섹션을 팀수익 위로 이동
- [ ] 실시간 글로벌 매출을 XPLAY 로드맵 다음으로 이동

## 실시간 글로벌 매출 백오피스 관리
- [ ] 백오피스에서 실시간 글로벌 매출 수동 수정 기능 (규칙 설정)
- [ ] 참여국가 수정 기능
- [ ] "누적거래량" → "누적 유동성" 텍스트 변경
- [ ] 투자시간 규칙성 조정 (자연스럽게)

## 수익 시뮬레이션 수수료 안내
- [ ] 시뮬레이션 산하에 "플랫폼 수수료 20% 미제외 금액" 표시
- [ ] "플랫폼 수수료는 변동 될 수 있습니다." 안내 추가

## 공지 에디터 개선
- [ ] 이미지 서버 업로드 + 서버에서 직접 송출
- [ ] 내용 에디터 크게 확대
- [ ] AI 연동: 내용 다듬기 기능
- [ ] 미리보기 후 송출하기 기능
- [ ] 자동 다국어 번역
...
## 출금 안내 텍스트 변경
- [ ] 출금 설명: "봇 수익율이 자동으로 6.25 USDT에 도달하면 20%를 제외하고 자동으로 지갑으로 입금됩니다. 자금은 모두 BNB체인 중심이지만 다양한 체인을 사용합니다."
